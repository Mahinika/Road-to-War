...
func regenerate_resources(hero_id: String, delta: float):
	...
	if current_hp < max_hp:
		...
		var cm = get_node_or_null("/root/CombatManager")
		var in_combat = cm.in_combat if cm else false
		var regen_percent = 0.02 if not in_combat else 0.005
	...
	if get_resource_type(hero_id) == "rage":
		...
		var decay_rate = 6.0 if in_combat else 18.0 # rage per second
