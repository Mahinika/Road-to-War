...
- `ShopManager.gd` - Shop encounters and purchasing
...
- Files: `classes.json`, `specializations.json`, `talents.json`, `stats-config.json`, `items.json`, `enemies.json`, `world-config.json`, `achievements.json`, `prestige-config.json`, `abilities.json`, `bloodlines.json`, `skill-gems.json`, `animation-config.json`, `keyframe-configs.json`
...
- `start-electron-dev.js` - Orchestrates Vite + Electron startup
- `dev-with-devtools.js` - Starts Vite with DevTools auto-open
- `check-logs.js` - Utility to check game logs
- `debug-enemies.js` - Enemy debugging script
- `e2e-connect.js` - E2E connection script
- `e2e-runner.js` - E2E test runner
- `expand-talents.js` - Talent expansion script

### `/tests` - Testing Suite
- `test-desktop-automation.js` - Desktop automation tests
- `test-electron.js` - Electron integration tests
...
- **`toolset/`** - Toolset consolidation planning
...
### `/dist` - Build Output
- Generated by `npm run build` or `npm run electron:build`
...
- `vite.config.js` - Vite build configuration (modify for build changes)
...
### Build System Files
- `vite.config.js` - Only modify for build configuration changes
- `electron-builder.json` - Only modify for packaging changes
...
### Testing Tools
1. **Unified Test Runner**: `npm test [--mode=<mode>]` → Run tests (5man, full, auto)
2. **Unit Tests**: `npm run test:unit` → Vitest unit tests for managers
3. **Integration Tests**: `npm run test:integration` → System integration tests
4. **E2E Tests**: `npm run test:ultimate` → Playwright E2E tests
5. **Coverage**: `npm run test:coverage` → Test coverage report
6. See `tools/README.md` for complete testing documentation

## Testing Framework

### Test Structure
- **Unit Tests**: `tests/unit/managers/` - Manager class tests (14 files)
- **Integration Tests**: `tests/integration/` - System interaction tests (5 files)
- **E2E Tests**: `tests/ultimate-test-suite.js` - Browser automation tests
- **Test Helpers**: `tests/utils/test-helpers.js` - Mock utilities

### Test Coverage
- **Status**: ✅ All managers have unit tests
- **Coverage**: 80%+ target (critical paths: 100%)
- **Framework**: Vitest with Phaser 3 mocks
- **Patterns**: See `docs/consolidated/testing/MANAGER_TESTING_PATTERNS.md`
...
### Browser
- Uses `localStorage` API (synchronous)
- SaveManager automatically detects environment

### Desktop (Electron)
- Uses Electron IPC (asynchronous via preload bridge)
- Save files stored in `%AppData%/RoadOfWar/saves/`
- SaveManager calls `window.electronAPI.saveGame()` / `loadGame()`

## Console Log Capture System

### Electron
- Real-time capture via `webContents.on('console-message')` event
- Logs written to `logs/game-output.log` file with timestamps

### Browser
- Console method override with localStorage persistence
- Logs accessible via `window.gameLogs` or `window.getGameLogs()`
- Export function: `window.exportGameLogs()`
